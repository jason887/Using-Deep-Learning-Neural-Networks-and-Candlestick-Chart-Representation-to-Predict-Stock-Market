...
def save_bottlebeck_features():
    ...
    np.save('bottleneck_features_train_vgg16_{}'.format(period), bottleneck_features_train)
...
def train_top_model():
    train_data = np.load('bottleneck_features_train_vgg16_{}.npy'.format(period))
    train_labels = np.array(
        [0] * (nb_train_samples // 2) + [1] * (nb_train_samples // 2))

    validation_data = np.load('bottleneck_features_validation_vgg16_{}.npy'.format(period))
