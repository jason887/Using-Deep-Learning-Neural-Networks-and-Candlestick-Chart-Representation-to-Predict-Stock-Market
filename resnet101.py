...
def build_model(SHAPE, nb_classes, bn_axis, seed=None):
    # [3, 4, 23, 3] resnet 101 architecture
    # error.
    if seed:
          np.random.seed(seed)
    ...
    for i in range(0,4):
        x = identity_block(x, 3, [128, 128, 512], stage=3, block='c{}'.format(i))
    ...
    for i in range(0,23):
        x = identity_block(x, 3, [256, 256, 1024], stage=4, block='d{}'.format(i))
    ...
    for i in range(0,3):
        x = identity_block(x, 3, [512, 512, 2048], stage=5, block='e{}'.format(i))
...
def main():
    ...
    model.save('{}epochs_{}period_{}dimension_resnet101_model.h5'.format(epochs,period_name[2],period_name[1]), overwrite=True)
    ...
    f_output.write('=======\n')
    f_output.write('{}epochs_{}period_{}dimension_resnet101\n'.format(epochs,period_name[2],period_name[1]))
    f_output.write('TN: {}\n'.format(tn))
    f_output.write('FN: {}\n'.format(fn))
    f_output.write('TP: {}\n'.format(tp))
    f_output.write('FP: {}\n'.format(fp))
