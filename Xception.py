...
top_model_weights_path = 'bottleneck_fc_model_Xception_{}.h5'.format(period)
...
def save_bottlebeck_features():
    ...
    np.save('bottleneck_features_train_Xception_{}'.format(period), bottleneck_features_train)
    ...
    np.save('bottleneck_features_validation_Xception_{}'.format(period),
            bottleneck_features_validation)


def train_top_model():
    train_data = np.load('bottleneck_features_train_Xception_{}.npy'.format(period))
    train_labels = np.array(
        [0] * (nb_train_samples // 2) + [1] * (nb_train_samples // 2))

    validation_data = np.load('bottleneck_features_validation_Xception_{}.npy'.format(period))
